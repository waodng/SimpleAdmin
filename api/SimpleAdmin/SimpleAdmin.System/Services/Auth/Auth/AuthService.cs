...
public class AuthService : IAuthService
{
    ...
    private async Task WriteTokenToRedis(LoginEvent loginEvent, LoginClientTypeEnum loginClientType)
    {
        ...
        //添加到token列表
        _simpleCacheService.HashAdd(CacheConst.Cache_UserToken, loginEvent.SysUser.Id, tokenInfos);
    }
    ...
    private void RemoveTokenFromRedis(LoginEvent loginEvent, LoginClientTypeEnum loginClientType)
    {
        ...
        if (tokenInfos != null)
        {
            ...
            {
                //从列表中删除
                _simpleCacheService.HashDel<List<TokenInfo>>(CacheConst.Cache_UserToken, new string[] { loginEvent.SysUser.Id });
            }
        }
    }
