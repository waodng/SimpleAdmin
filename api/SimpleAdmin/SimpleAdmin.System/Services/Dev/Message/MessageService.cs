...
public class MessageService : DbRepository<DevMessage>, IMessageService
{
    ...
    /// <inheritdoc />
    public async Task<SqlSugarPagedList<DevMessage>> MyMessagePage(MessagePageInput input, string userId)
    {
    ...
    /// <inheritdoc />
    public async Task DeleteMyMessage(BaseIdInput input, string userId)
    {
        var messageUserRep = ChangeRepository<DbRepository<DevMessageUser>>();//切换仓储
