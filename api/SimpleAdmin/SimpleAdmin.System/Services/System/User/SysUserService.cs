...
public class SysUserService : DbRepository<SysUser>, ISysUserService
{
    ...
    {
        //先从Redis拿
        var userId = _simpleCacheService.HashGetOne<string>(CacheConst.Cache_SysUserPhone, phone);
        if (string.IsNullOrEmpty(userId))
        {
    ...
    /// <inheritdoc/>
    public async Task<SysUser> GetUserById(string userId)
    {
        //先从Redis拿
        var sysUser = _simpleCacheService.HashGetOne<SysUser>(CacheConst.Cache_SysUser, userId);
        if (sysUser == null)
        {
            ...
            if (sysUser != null)
            {
                ...
                //插入Redis
                _simpleCacheService.HashAdd(CacheConst.Cache_SysUser, sysUser.Id, sysUser);
            }
        }
        return sysUser;
    }
    ...
    {
        //先从Redis拿
        var userId = _simpleCacheService.HashGetOne<string>(CacheConst.Cache_SysUserAccount, account);
        if (string.IsNullOrEmpty(userId))
        {
            //单查获取用户账号对应ID
    ...
    public async Task<List<DataScope>> GetPermissionListByUserId(string userId, string orgId)
    {
        ...
        if (sysRelations.Count == 0)//如果有表示用户单独授权了不走用户角色
        {
            ...
            {
                //获取角色权限信息
                sysRelations = await _relationService.GetRelationListByObjectIdListAndCategory(roleIdList.Select(it => it.TargetId).ToList(), CateGoryConst.Relation_SYS_ROLE_HAS_PERMISSION);
            }
        }
        ...
        {
            HashSet<string> scopeSet = new HashSet<string>();//定义不可重复列表
    ...
    {
        DeleteUserFromRedis(new List<string>
        {
            userId
        });
    }
    ...
    {
        var userIds = ids.ToArray();//id转string列表
        ...
        if (sysUsers.Count > 0)
        {
            ...
            //删除账号
            _simpleCacheService.HashDel<string>(CacheConst.Cache_SysUserAccount, accounts);
            //删除手机
            if (phones != null)
                _simpleCacheService.HashDel<string>(CacheConst.Cache_SysUserPhone, phones);
        }
    }
    ...
    private async Task CheckInput(SysUser sysUser)
    {
        ...
        if (!string.IsNullOrEmpty(accountId) && accountId != sysUser.Id)
        ...
        if (!string.IsNullOrEmpty(sysUser.Phone))
        {
            ...
            if (!string.IsNullOrEmpty(phoneId) && sysUser.Id != phoneId)//判断重复
